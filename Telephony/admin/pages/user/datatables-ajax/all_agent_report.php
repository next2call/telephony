...
$searchQuery = "";
if ($searchValue != '') {
    $searchQuery = " AND (ll.id LIKE '%$searchValue%' OR 
                         ll.user_name LIKE '%$searchValue%' OR 
                         ll.log_in_time LIKE '%$searchValue%' OR 
                         ll.log_out_time LIKE '%$searchValue%' OR 
                         ll.status LIKE '%$searchValue%')";
}
...
if ($serch_type == 'date') {
    $condition = "DATE(ll.log_in_time) BETWEEN '$fromdate' AND '$todate'";
} elseif ($serch_type == 'agent') {
    $condition = "ll.user_name IN ('$agentid')";
} elseif ($serch_type == 'date-agent') {
    $condition = "DATE(ll.log_in_time) BETWEEN '$fromdate' AND '$todate' AND ll.user_name IN ('$agentid')";
} else {
    $condition = "1=1";
}
...
$totalRecordsQuery = "SELECT COUNT(*) as allcount 
                      ...
                      WHERE ll.admin = '$user' AND $condition";
...
$totalRecordwithFilterQuery = "SELECT COUNT(*) as allcount 
                               FROM login_log AS ll 
                               WHERE ll.admin = '$user' AND $condition $searchQuery";
...
SELECT 
    ...
    ll.log_in_time,
    ll.log_out_time,
    DATE(ll.log_in_time) AS date_only,
    COUNT(DISTINCT bt.break_name) AS no_of_day_break,
    COUNT(DISTINCT cdr.id) AS call_count
FROM 
    login_log AS ll
LEFT JOIN 
    break_time AS bt 
    ON DATE(bt.start_time) = DATE(ll.log_in_time) 
    AND bt.user_name = ll.user_name
LEFT JOIN 
    cdr 
    ON DATE(cdr.start_time) = DATE(ll.log_in_time) 
    AND (cdr.call_from = ll.user_name OR cdr.call_to = ll.user_name)
WHERE 
    ll.admin = '$user' AND $condition $searchQuery
GROUP BY 
    ll.user_name, ll.log_in_time, ll.log_out_time
ORDER BY 
...
while ($row = mysqli_fetch_assoc($empRecords)) {
    $data[] = array(
        ...
        "no_of_day_break" => $row['no_of_day_break'],
