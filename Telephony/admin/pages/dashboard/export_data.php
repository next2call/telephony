...
if ($serch_type == 'date') {
    $query = "WITH RankedData AS (
        SELECT cdr.*, TIME_FORMAT(SEC_TO_TIME(cdr.dur), '%H:%i:%s') AS dur_formatted, 
               users.full_name, 
               CASE 
                   WHEN cdr.call_to = users.user_id THEN users.campaigns_id 
                   ELSE NULL 
               END AS campaigns_id,
               CASE 
                   WHEN cdr.call_to = users.user_id THEN compaign_list.compaignname 
                   ELSE NULL 
               END AS compaignname,
    ...
    $query = "WITH RankedData AS (
        ...
        LEFT JOIN users ON users.user_id = cdr.call_to 
        LEFT JOIN compaign_list ON compaign_list.compaign_id = users.campaigns_id 
        WHERE $lavel_cam AND $lavel_condition $condition_one
    ) 
    SELECT * FROM RankedData WHERE rn = 1 ORDER BY id DESC";
    ...
    $query = "WITH RankedData AS (
        ...
        LEFT JOIN compaign_list ON compaign_list.compaign_id = users.campaigns_id 
        WHERE DATE(cdr.start_time) BETWEEN '$fromdate' AND '$todate' 
          AND $lavel_cam AND $lavel_condition $condition_one
    ) 
    SELECT * FROM RankedData WHERE rn = 1 ORDER BY id DESC";
    ...
    $query = "WITH RankedData AS (
        ...
        LEFT JOIN users ON users.user_id = cdr.call_to 
        LEFT JOIN compaign_list ON compaign_list.compaign_id = users.campaigns_id 
        WHERE $lavel_cam AND $lavel_agent AND $lavel_condition $condition_one
    ) 
    SELECT * FROM RankedData WHERE rn = 1 ORDER BY id DESC";
    ...
    $query = "WITH RankedData AS (
        ...
        LEFT JOIN compaign_list ON compaign_list.compaign_id = users.campaigns_id 
        WHERE DATE(cdr.start_time) BETWEEN '$fromdate' AND '$todate' 
          AND $lavel_cam AND $lavel_agent AND $lavel_condition $condition_one
    ) 
    SELECT * FROM RankedData WHERE rn = 1 ORDER BY id DESC";
} else {
    $query = "WITH RankedData AS (
