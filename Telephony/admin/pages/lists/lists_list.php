...
if($user_level == '9'){
    ...
    $usersql2 = "SELECT * FROM `lists` WHERE ADMIN IN ($admin_ids_str)";
}else{
 ...
 //  } // check select user select or no select
  ...
  if (isset($_POST["import"])) {
    ...
    if (!move_uploaded_file($_FILES['excel']['tmp_name'], $targetDirectory)) {
        echo showAlert('Failed', 'File upload failed. Please try again.', 'error');
        exit;
    }
    ...
    foreach ($reader as $key => $row) {
        ...
        if (mysqli_num_rows($checkResult) == 0) {
            ...
            $insertQuery = "
                ...
                (company_name, employee_size, industry, country, city, department, designation, email, name, phone_number, phone_2, phone_3, phone_code, admin, ins_date, dial_status, list_id, campaign_Id) 
                VALUES 
                ('$company_name', '$employee_size', '$industry', '$country', '$city', '$department', '$designation', '$email', '$name', '$phone_number', '$phone2', '$phone3', '$phone_code', '$Adminuser', '$ins_date', 'NEW', '$list_id', '$campaign_name')
            ";
    ...
    // Calculate success and duplicate rates
    $successPercentage = ($totalRows > 0) ? ($successCount / $totalRows) * 100 : 0;
    ...
    window.onload = function() {
        Swal.fire({
            ...
            html: "Successfully inserted: <b>' . $successCount . '</b><br>Duplicates skipped: <b>' . $duplicateCount . '</b>",
            toast: true,
            position: "top-end",
...
<div>
    <div class="show-users">
        ...
        <a href="<?= $upload_dd_f ?>dashboard/uploads/vicidial_data.csv" download><i class="fa fa-arrow-circle-o-down" style="font-size:20px;"> Download</i></a>
    </div>  
...
<?php
                    ...
                    if ($user_level != 2) {
                          echo "<i class='fa fa-trash cursor_p text-danger mb-2 delete_your_list' style='font-size:20px;' data-id='" . $row['ID'] . "' title='You can click and remove Your List'></i>";
                      }
                    echo "</td>";
                    echo '</tr>';
...
<script>
  ...
  function updateLeadCount() {
    // Loop through each element with the class 'lead_count' and update it
    $('.lead_count').each(function() {
      var element = $(this);
      var list_id = element.data("list_id"); // Get the list_id from the data attribute

      // Make AJAX request to get the lead count for this list_id
      $.ajax({
        url: "pages/lists/leadcount_ajaxfile.php", // Your PHP file that returns the lead count
        data: { list_id: list_id },
        type: "POST",
        dataType: "json",
        success: function(res) {
          // Assuming the response contains a 'count' field with the lead count
          var leadCount = res.count || '0';  // Default to 0 if no count is returned
          element.html(leadCount);  // Update the lead count in the corresponding <td>
        },
