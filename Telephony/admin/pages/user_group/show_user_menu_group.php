...
if($menu_id){
    $sect1 = "SELECT vicidial_menu_group.* FROM vicidial_menu_group JOIN compaign_list ON compaign_list.compaign_id=vicidial_menu_group.campaign_id WHERE vicidial_menu_group.menu_id='$menu_id' AND compaign_list.admin='$Adminuser'";
    // $sect1 = "SELECT * FROM vicidial_menu_group WHERE menu_id='$menu_id'";
    $sel_q = mysqli_query($con, $sect1);
}else{
    $sect1 = "SELECT vicidial_menu_group.* FROM vicidial_menu_group JOIN compaign_list ON compaign_list.compaign_id=vicidial_menu_group.campaign_id WHERE compaign_list.admin='$Adminuser'"; 
$sel_q = mysqli_query($con, $sect1);
...
// $row_s1 = mysqli_fetch_assoc($sel_q);
// $camp_id = $row_s1['compaign_id'];
...
// die();
    $select1 = "SELECT vicidial_menu_group.* FROM `vicidial_menu_group` JOIN compaign_list ON compaign_list.compaign_id=vicidial_menu_group.campaign_id WHERE (vicidial_menu_group.group_id='$group_id' OR vicidial_menu_group.press_key='$presskey') AND compaign_list.admin='$Adminuser'";
//   die();  
...
// Create the SQL query string
   $sql_New = "INSERT INTO `vicidial_menu_group`(`group_id`, `group_name`, `campaign_id`, `press_key`, `menu_id`) VALUES ('$group_id','$group_name','$campaign_id','$presskey','$menu_id')";
// die();
$insert =mysqli_query($con, $sql_New);
    ...
    if($insert){
        ...
        window.onload = function() {
            ...
            window.location.href = "' . $admin_ind_page . '?c=user_group&v=show_user_menu_group&id=' . $id . '";
            });
        };                
        </script>';
        // exit; // Ma
...
//    echo "</br>";
    ...
    $menu_id = $_POST['menu_id'];
    $presskey = $_POST['presskey'];
 $select1 = "SELECT * FROM `vicidial_menu_group` WHERE id='$ind_id'";
    $sel_query1 = mysqli_query($con, $select1);
    if (mysqli_num_rows($sel_query1) > 0) {
       $r_data = mysqli_fetch_assoc($sel_query1);
        $o_pressk = $r_data['press_key'];
       $sql_New = "UPDATE `vicidial_menu_group` SET `press_key`='$o_pressk' WHERE press_key='$presskey'";
       mysqli_query($con, $sql_New);
    }

// Create the SQL query string

  $sql_New = "UPDATE `vicidial_menu_group` SET `group_name`='$group_name',`campaign_id`='$campaign_id',`press_key`='$presskey',`menu_id`='$menu_id' WHERE id='$ind_id'";
                    ...
                    <div class="horizontal-container">
                                                ...
                                                <div class="dropdown">
                                                    <img src="../assets/images/common-icons/addclint.png" alt="add_icon"
                                                        ...
                                                        class=" shadow-sm best_font dropdown-toggle cursor_p"
                                                        data-toggle="dropdown" aria-haspopup="true"
                                                        title="Assign Department to agents" data-id="<?= $row['id'] ?>"
                                               ...
                                               ?>
                                                        ...
                                                        <table>
                                                            <thead>
                                                                <tr>
                                                                    <th>
                                                                        <!-- <input type="checkbox"
                                                                            class='cursor-p check-all'
                                                                            id='<?= $row['user_id'] ?>'
                                                                            data-uniqnumber='<?= $row['user_id'] ?>'> -->
                                                                    </th>
                                                                    <th class="text-primary">Agent Name</th>
                                                                    <th class="text-primary">Agent ID</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                <?php 
                                                     ...
                                                     ?>
                                                                ...
                                                                <tr>
                                                                    <td><input type="checkbox"
                                                                            value="<?= $agent_row['id'] ?>"
                                                                            class='cursor-p agent <?= $row['user_id'] ?> mr-2'>
                                                                    </td>
                                                                    <td class="ml-5"><?= $agent_row['full_name'] ?></td>
                                                                ...
                                                                <tr>
                                                                    <td colspan='2'><button
                                                                            ...
                                                                            data-cnumber="<?= $row['group_id'] ?>" data-cname="<?= $row['campaign_id'] ?>" data-press_key="<?= $row['press_key'] ?>">assign</button>
                                                                    </td>
                                                ...
                                                <div class="dropdown">      
                                                    <img src="../assets/images/common-icons/assign_agent.png" alt="add_icon"
                                                        style="height:20px; width:27px;" class="cursor_p"
                                                        data-toggle="dropdown" aria-haspopup="true"
                                                        title="Review agents assigned Department">
                                                        <div class="dropdown-menu"
                                                        style="overflow: scroll; height: <?= $hieght_scrol ?>; width:auto"
                                                        aria-labelledby="select-btn">
                                                        <table>
                                                            <thead>
                                                                ...
                                                                <tr>
                                                                    <th class="text-primary">Assign Agent</th>
                                                                    <!-- <th class="text-primary">Agent ID</th> -->
                                                                </tr>
                                                            </thead>
                                                            <tbody>
...
<tr>
                                                ...
                                                <a href="?c=user_group&v=show_menu_group_agent&group_id=<?= $group_id ?>&id=<?= $id ?>">
                                                <!-- <a href="?c=user_group&v=bulk_group_change&group_id=<?= $group_id ?>&id=<?= $id ?>"> -->
            ...
            <form action="" method="post">
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="my-input-with-help col-6">
                            <div class="form-group my-input">
                                <input type="text" class="form-control" id="new-user-menu-id" name="menu_id"
                                    aria-describedby="user_group" value="<?= $menu_id; ?>" readonly>
                        ...
                        <?php } else{ ?>
                            <div class="my-dropdown-with-help col-6">
                            <div class="my-dropdown">
                                <select name="menu_id" id="new-user-group-menu-id" required> 
                                    <?php 
                                    $select1 = "SELECT menu_ivr_tbl1.* FROM menu_ivr_tbl1 JOIN compaign_list ON compaign_list.compaign_id = menu_ivr_tbl1.campaign_id WHERE compaign_list.admin='$Adminuser'";
                                    $sel_query1 = mysqli_query($con, $select1);
                                    while($roow_camp = mysqli_fetch_assoc($sel_query1)){ 
                                        $menu_name = $roow_camp['menu_name'];
                                        $menu_id = $roow_camp['menu_id'];
                                        ?>
                                    <option value="<?= $menu_id ?>"><?= $menu_name ?></option>
                                    <?php } ?>
                                </select>
                                <i class="fa fa-caret-down my-dropdown-arrow" aria-hidden="true"></i>
                                <label for="source_campaign_id">Menu Name</label>
                            </div>
                        </div>
                        <?php } ?>
                        ...
                        <div class="my-dropdown-with-help col-6">
                            <div class="my-dropdown">
                                <select id="newpresskey" name="presskey" required>
                                    <?php 
                                    $select1 = "SELECT vicidial_menu_group.* FROM vicidial_menu_group JOIN compaign_list ON compaign_list.compaign_id=vicidial_menu_group.campaign_id WHERE compaign_list.admin='$Adminuser'";
                                    $sel_query1 = mysqli_query($con, $select1);
                                    while($roow_camp = mysqli_fetch_assoc($sel_query1)){ 
...
$(document).ready(function() {
    $(document).on("click", ".update_gp", function() {
        ...
        $.ajax({
            url: "pages/new_action/get_menu_group.php",
            type: "POST",
            data: { id: id },
...
$(document).ready(function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = "pages/new_action/remove_menu_group.php?id=" + id + "&group_id=" + group_id;
        }
    });
  });
