...
if(!empty($_REQUEST['id'])){
    $sel = "SELECT * FROM menu_ivr_tbl1 WHERE campaign_id='$camp_id'";
    } else {
        $sel = "SELECT menu_ivr_tbl1.* FROM vicidial_group
    JOIN compaign_list ON compaign_list.compaign_id=menu_ivr_tbl1.campaign_id WHERE compaign_list.admin='$Adminuser'";
    }
...
if(isset($_POST['submit'])){
    ...
    $select1 = "SELECT menu_ivr_tbl1.* FROM `menu_ivr_tbl1` JOIN compaign_list ON compaign_list.compaign_id=menu_ivr_tbl1.campaign_id WHERE (menu_ivr_tbl1.menu_id='$group_id' OR menu_ivr_tbl1.press_key='$presskey') AND compaign_list.admin='$Adminuser'";
...
// Create the SQL query string
  $sql_New = "INSERT INTO `menu_ivr_tbl1`(`menu_id`, `menu_name`, `campaign_id`, `press_key`, `ivr_file`, `ivr`) VALUES ('$group_id','$group_name','$campaign_id','$presskey','$menu_ivr_path','$group_wise')";
...
$insert =mysqli_query($con, $sql_New);
    ...
    if($insert){
        ...
        window.onload = function() {
            ...
            window.location.href = "' . $admin_ind_page . '?c=user_group&v=ivr_menu_group&id=' . $id . '";
            });
        };                
        </script>';
        // exit; // Ma
...
if(isset($_POST['update'])){
 ...
 $select1 = "SELECT * FROM `menu_ivr_tbl1` WHERE id='$ind_id'";
    $sel_query1 = mysqli_query($con, $select1);
    if (mysqli_num_rows($sel_query1) > 0) {
       $r_data = mysqli_fetch_assoc($sel_query1);
        $o_pressk = $r_data['press_key'];
       $sql_New = "UPDATE `menu_ivr_tbl1` SET `press_key`='$o_pressk' WHERE press_key='$presskey'";
                 ...
                 <?php 
                   ...
                   <?php if($ivr_type == '1'){ ?> 
                    ...
                    <?php }else if($ivr_type == '2'){ ?> 
                        <i class="fa fa-repeat sec_wised_menu cursor_p" style="font-size:20px; color:blue;" data-toggle="modal" data-menu_id="<?= $row['menu_id'] ?>" data-camp_id="<?= $row['campaign_id'] ?>" data-target="#add-menu-ivr"></i> &nbsp;&nbsp;
                        <i class="fa fa-pencil-square update_gp cursor_p" aria-hidden="true" data-toggle="modal" data-target="#update-user-group" data-id="<?= $row['id'] ?>" style="font-size:20px; color:blue;"></i>
                        &nbsp;&nbsp;
                        <i class="fa fa-trash remove_this_agent cursor_p" aria-hidden="true" style="font-size:24px; color:red;" data-id="<?= $row['id'] ?>" data-group_id="<?= $row['menu_id'] ?>" data-pid="<?= $id?>" title='You can click here to remove agent'></i>
                    <?php } ?>
...
$(document).ready(function() {
    $(document).on("click", ".update_gp", function() {
        ...
        $.ajax({
            url: "pages/new_action/get_menu_groupone.php",
            type: "POST",
            data: { id: id },
...
$(document).ready(function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = "pages/new_action/remove_ivr_menu_group.php?id=" + id + "&group_id=" + group_id + "&pid=" + pid;
        }
    });
  });
...
<script>
$(document).ready(function() {
    $(document).on("click", ".sec_wised_menu", function() {
        var menu_id = $(this).data("menu_id");
        ...
        // alert(menu_id);
        window.location.href = "?c=user_group&v=ivr_menu_group_one&menu_id=" + menu_id +"&camp_id=" + camp_id;
    });
});
</script>
