...
// $con = new mysqli("localhost", "mehtauser", "mehtapassw", "mehta");
$user = new user();
...
if(isset($_POST['create_speech'])){
    ...
    fclose($file);
// exec('asterisk -rx "moh reload"', $output, $return_var);
...
<div>
    <div class="show-users ml-5">
        ...
        <div class="table-responsive my-table ml-5">
            <div class="table-top">
                <a class="data_btn" href="#create-speech" data-toggle="modal" data-target="#create-speech"><i
            ...
            <table class="all-user-table table table-hover">
                ...
                <tbody>
                    ...
                    while ($usersrow = mysqli_fetch_array($usersresult)) {
                        ...
                        echo '<tr>';
                        echo '<td>' . $sr . '</td>';
                        // echo '<td>' . $usersrow['file_name'] . '</td>';
                        echo '<td>' . $usersrow['type'] . '</td>';
                        echo '<td>' . $usersrow['campaign_name'] . '</td>';
                        echo '<td><audio controls><source src="/'.$main_folder.'/admin/' . $usersrow['file_name'] . '" type="audio/wav">Your browser does not support the audio element.</audio></td>';
                        echo '<td>' . $usersrow['date'] . '</td>';
                        ?>
                    <td>
                        <!-- <span class="badge bg-danger cursor_p text-white show_break_report"
                            data-id="<?php echo $usersrow['id']; ?>" data-type="<?php echo $usersrow['type']; ?>"
                            title="You can click here to remove ivr file">Remove</span> -->

                        <i class="fa fa-pencil-square cursor_p assign_speech" aria-hidden="true" data-toggle="modal"
                            data-target="#assign_speech" data-id="<?= $usersrow['id'] ?>"
                            data-type="<?php echo $usersrow['type']; ?>"
                            data-file_name="<?php echo $usersrow['file_name']; ?>"
