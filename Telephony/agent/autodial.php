...
if ((mysqli_num_rows($qur) > 0)) {
    ...
    $sel = "
        ...
        WHERE u.admin = '$user_admin' AND (u.username = '$user' OR u.username = '') AND u.dial_status = 'NEW' AND u.campaign_Id = '$campaign_id' AND l.call_to IS NULL ORDER BY u.id DESC
        LIMIT 1
    ";
    $qur = mysqli_query($con, $sel); 
    if (mysqli_num_rows($qur) > 0) {
        ...
        $sel_two = "
            ...
            FROM upload_data u
            LEFT JOIN live l ON u.phone_number = l.call_to
            WHERE u.username = '$user' AND u.dial_status = 'NEW' AND u.campaign_Id = '$campaign_id' AND l.call_to IS NULL ORDER BY u.id DESC
            LIMIT 1
        ";
